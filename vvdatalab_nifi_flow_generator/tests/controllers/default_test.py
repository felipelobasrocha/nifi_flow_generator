...
class DefaultTest(unittest.TestCase):
    ...
    def test_default(self):
        ...
        parameter = ''
        file = os.path.split(os.path.dirname(__file__))[0].replace('/tests', '') + '/static/data/default.json'
        with open(file) as json_file:
            data = json.load(json_file)
            parameter = json.dumps(data)

        response = self.app.post('/default', data=dict(process_group=parameter))

        self.assertEqual(response.status_code, 200)

    def test_default_db2_hive(self):

        self.setup()

        parameter = ''
        file = os.path.split(os.path.dirname(__file__))[0].replace('/tests', '') + '/static/data/db2_hive/ctf_sgo_fma_pgt.json'
        with open(file) as json_file:
            data = json.load(json_file)
            parameter = json.dumps(data)
